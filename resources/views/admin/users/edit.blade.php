...
@section('content')
    ...
    @endif
    <main class="m-5 ">
        <form action="{{route('admin.user.update',$user)}}" method="post" class=" " enctype="multipart/form-data">
            @csrf

            <h1 class="h2 mb-4  fw-normal text-capitalize ">Edit Admin</h1>

            <div class=" mb-2">
                <label for="name" class="pb-2">Name</label>
                <input type="text" class="form-control @error('name') border-danger @enderror " value="{{$user->name}}"
            ...
            <div class=" mb-2">
                ...
                <input type="email" class="form-control @error('email') border-danger @enderror "
                       value="{{$user->email}}" id="email" name="email" placeholder="mo@bu.edu.sa">
            </div>
            @error('email')
            <span class="text-danger">{{$message}}</span>
            @enderror
